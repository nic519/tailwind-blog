...
import { useNavigationTracking } from '@/components/nav/useNavigationTracking'

export default function NavLayout({ navItems }: { navItems: NavData }) {
  const [isLoading, setIsLoading] = useState(true)
  const navigation = useNavigationTracking(navItems)
  ...
  return (
    <GradientBackground enableGrid={true} className="min-h-screen">
      <div className="flex">
        <aside
          className="w-52 backdrop-blur-md bg-white/70 dark:bg-gray-950/20
        ...
        >
          <NavTOC navItems={navItems} {...navigation} />
        </aside>
                          ...
                          {section.itemNav ? (
                            ...
                            <div className="space-y-8">
                              {section.nav?.map((subSection) => {
                                return (
                                  <section key={subSection.id} id={subSection.id} className="mb-8">
